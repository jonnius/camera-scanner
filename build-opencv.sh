...
BUILD_ARGS="$BUILD_ARGS -DBUILD_LIST=core,imgproc,highgui,imgcodecs"
# Disable some stuff to reduce build time
BUILD_ARGS="$BUILD_ARGS -DBUILD_EXAMPLES=OFF"
BUILD_ARGS="$BUILD_ARGS -DBUILD_DOCS=OFF"
BUILD_ARGS="$BUILD_ARGS -DBUILD_PERF_TESTS=OFF"
BUILD_ARGS="$BUILD_ARGS -DBUILD_TESTS=OFF"
BUILD_ARGS="$BUILD_ARGS -DBUILD_OPENCV_APPS=OFF"
# Enable some stuff to speed it up
BUILD_ARGS="$BUILD_ARGS -DWITH_TBB=OFF"
BUILD_ARGS="$BUILD_ARGS -DWITH_OPENMP=OFF" # we should try if this speeds up the image processing
BUILD_ARGS="$BUILD_ARGS -DWITH_IPP=OFF"
BUILD_ARGS="$BUILD_ARGS -DWITH_NVCUVID=OFF"
BUILD_ARGS="$BUILD_ARGS -DWITH_CUDA=OFF"
BUILD_ARGS="$BUILD_ARGS -DWITH_CSTRIPES=OFF"
BUILD_ARGS="$BUILD_ARGS -DWITH_OPENCL=OFF"
# Static libs
BUILD_ARGS="$BUILD_ARGS -DBUILD_SHARED_LIBS=OFF"
